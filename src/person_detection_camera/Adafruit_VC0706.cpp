...
#endif
    hwSerial->begin(baud, SERIAL_8N1, 18, 19);
  return reset();
}
...
*/
/**************************************************************************/
uint8_t *Adafruit_VC0706::readPicture(uint32_t n) {
  uint8_t args[] = {0x0C,
                    0x0,
                    0x0A,
  ...
  if (readResponseLong(n + 5, CAMERADELAY) == 0)
    return 0;
...
uint32_t Adafruit_VC0706::readResponseLong(uint32_t numbytes, uint8_t timeout) {
  uint32_t counter = 0;
  bufferLen = 0;
  int avail;
